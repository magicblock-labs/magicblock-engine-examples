...
export async function ensureUserBalance(
  ...
  try {
    ...
    if (userBalance < MIN_BALANCE_SOL * LAMPORTS_PER_SOL) {
      try {
        ...
        console.log(
          "Airdrop request failed (network may not support airdrops):",
          airdropErr instanceof Error ? airdropErr.message : airdropErr,
        );
        console.log(
          "Note: Ensure user has sufficient SOL before running tests",
        );
      }
    } else {
      console.log(
  ...
  if (finalBalance === 0) {
    console.log(
      "WARNING: User has 0 SOL. Tests may fail. Fund the account manually:",
    );
