...
export const AdminActions: React.FC<AdminActionsProps> = ({ selectedDistributor }) => {
  ...
  // Each escrow authority gets its own DLP "ephemeral balance" PDA on base:
  //  - reward distributor escrow: pays ATA rent for reward-claim callbacks
  //    (transfer_spl_token / transfer_programmable_nft sourced from the
  //    RewardDistributor)
  //  - whitelist distributor escrow: pays ATA rent for whitelist_transfer
  //    payout callbacks (e.g. buybacks). Funding one does NOT cover the other.
  const whitelistDistributor = targetDistributor
  ...
  const escrowPda = targetDistributor
    ...
    : null;
  const whitelistEscrowPda = whitelistDistributor
    ? deriveEphemeralBalancePda(whitelistDistributor, DEFAULT_ESCROW_INDEX)[0]
    : null;
  // Which escrow the top-up modal targets
  const [escrowTarget, setEscrowTarget] = useState<"distributor" | "whitelist">(
    "distributor"
  );
  ...
  const [whitelistEscrowBalance, setWhitelistEscrowBalance] = useState<{
    totalLamports: number;
    loading: boolean;
  } | null>(null);
  ...
  // Fetch both escrow PDAs' lamport balances from base when the modal opens
  useEffect(() => {
    if (activeModal !== "topUpEphemeralBalance" || !escrowPda) {
      setEscrowBalance(null);
      setWhitelistEscrowBalance(null);
      return;
    }

    let cancelled = false;
    setEscrowBalance({ totalLamports: 0, loading: true });
    setWhitelistEscrowBalance({ totalLamports: 0, loading: true });

    const fetchBalances = async () => {
      try {
        const solEndpoint = resolveEndpoint(connection.rpcEndpoint, "solana");
        const solConn = new Connection(solEndpoint, "confirmed");
        const [distLamports, wlLamports] = await Promise.all([
          solConn.getBalance(escrowPda, "confirmed"),
          whitelistEscrowPda
            ? solConn.getBalance(whitelistEscrowPda, "confirmed")
            : Promise.resolve(0),
        ]);
        if (!cancelled) {
          setEscrowBalance({ totalLamports: distLamports ?? 0, loading: false });
          setWhitelistEscrowBalance({ totalLamports: wlLamports ?? 0, loading: false });
        }
      } catch {
        if (!cancelled) {
          setEscrowBalance({ totalLamports: 0, loading: false });
          setWhitelistEscrowBalance({ totalLamports: 0, loading: false });
        }
      }
    };
    ...
    void fetchBalances();
    return () => { cancelled = true; };
  }, [activeModal, escrowPda?.toBase58(), whitelistEscrowPda?.toBase58(), connection.rpcEndpoint]);
  ...
  useEffect(() => {
    const activeBalance =
      escrowTarget === "whitelist" ? whitelistEscrowBalance : escrowBalance;
    if (activeModal !== "topUpEphemeralBalance" || !activeBalance || activeBalance.loading) return;
    ...
    // Remaining redemptions bound both flows: each redemption may create one
    // reward ATA (distributor escrow), and each redeemed reward may later be
    // bought back, creating one payout ATA (whitelist escrow).
    const totalRemaining = (rewardList?.rewards ?? []).reduce(
      (sum: number, r: any) => sum + Math.max(0, Number(r.redemptionLimit) - Number(r.redemptionCount)),
      0
    );
    ...
    const neededLamports = Math.ceil(totalRemaining * LAMPORTS_PER_REDEMPTION * 1.2);
    const deficitLamports = Math.max(0, neededLamports - activeBalance.totalLamports);

    const sol = deficitLamports / 1e9;
    const suggested = deficitLamports === 0 ? "0" : sol.toFixed(9).replace(/\.?0+$/, "");

    setForms((prev) => ({ ...prev, topUpEphemeralBalance: { amountSol: suggested } }));
  }, [activeModal, escrowTarget, escrowBalance, whitelistEscrowBalance, rewardList?.rewards]);
  ...
  const handleTopUpEphemeralBalance = async () => {
    ...
    const lamports = BigInt(Math.floor(solAmount * 1_000_000_000));
    const authority =
      escrowTarget === "whitelist" && whitelistDistributor
        ? whitelistDistributor
        : targetDistributor;
    const result = await topUpEphemeralBalance(lamports, authority);
    await handleTransactionResult(
      result,
      escrowTarget === "whitelist"
        ? "Top Up Whitelist Escrow"
        : "Top Up Distributor Escrow",
      () => {
  ...
  return (
    <div className="space-y-4">
      ...
      <TransactionModal
        ...
        title="Top Up Escrow"
        description="Fund an escrow PDA on base — the reward distributor and whitelist distributor each have their own escrow paying ATA rent for their scheduled transfer callbacks"
        loading={localStatus.loading}
        error={localStatus.error}
        signature={localStatus.signature}
        endpoint={localStatus.endpoint || connection.rpcEndpoint}
      ...
      >
        <div className="space-y-3">
          {/* Escrow picker — the reward distributor and the whitelist
              distributor each have their own DLP escrow; funding one does
              not cover the other. */}
          {(
            [
              {
                ...
                hint: "Pays ATA rent for reward-claim callbacks (SPL/NFT transfers)",
                authorityLabel: "Authority (Reward Distributor PDA)",
                authority: targetDistributor,
                escrow: escrowPda,
              ...
              },
              {
                ...
                hint: "Pays ATA rent for whitelist_transfer payout callbacks (e.g. buybacks)",
                authorityLabel: "Authority (Whitelist Distributor PDA)",
                authority: whitelistDistributor,
                escrow: whitelistEscrowPda,
                balance: whitelistEscrowBalance,
              },
            ]
          ).map(({ key, label, hint, authorityLabel, authority, escrow, balance }) => {
            // One ATA creation costs ~0.00204 SOL — below that, the next
            // callback that needs a fresh ATA will fail on base.
            const LAMPORTS_PER_TOKEN_ACCOUNT = 2_039_280;
            const isLow =
              balance != null && !balance.loading &&
              balance.totalLamports < LAMPORTS_PER_TOKEN_ACCOUNT;
            const selected = escrowTarget === key;
            return (
              <button
                key={key}
                type="button"
                onClick={() => setEscrowTarget(key)}
                className={`w-full text-left rounded border p-3 text-xs transition ${
                  selected
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  <span className={`font-medium ${selected ? "text-emerald-300" : "text-gray-300"}`}>
                    {label}
                  </span>
                  {balance?.loading ? (
                    <span className="text-gray-500 italic">Fetching…</span>
                  ) : balance ? (
                    <span className={`font-mono ${isLow ? "text-red-400" : "text-white"}`}>
                      {(balance.totalLamports / 1e9).toFixed(9)} SOL
                    </span>
                  ) : (
                    <span className="text-gray-500 italic">—</span>
                  )}
                </div>
                <p className="text-gray-500 mb-2">{hint}</p>
                {authority && (
                  <div className="mb-1">
                    <p className="text-gray-400 mb-0.5">{authorityLabel}</p>
                    <CopyableAddress address={authority.toBase58()} />
                  </div>
                )}
                {escrow && (
                  <div>
                    ...
                    <CopyableAddress address={escrow.toBase58()} />
                  </div>
                )}
                {isLow && (
                  <p className="text-red-400 mt-2">
                    ⚠️ Below the ~0.00204 SOL needed to create one token
                    account — callbacks that must create a destination ATA
                    will fail until this escrow is topped up.
                  </p>
                )}
              </button>
            );
          ...
          <div className="bg-gray-800 p-2 rounded text-xs text-gray-400 space-y-1">
            ...
            <p>Your wallet pays. The escrow PDA stays undelegated on base — only its authority (the selected distributor PDA) is used to derive it, no signature from the distributor is needed for top-ups.</p>
          </div>
        </div>
      </TransactionModal>
