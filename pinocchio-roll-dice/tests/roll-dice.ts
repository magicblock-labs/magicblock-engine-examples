...
describe("roll-dice", () => {
  ...
  const programId = readProgramId("roll_dice");
  const oracleQueue = DEFAULT_BASE_QUEUE;
  ...
  it("Initialized player!", async () => {
    ...
    await baseConnection.confirmTransaction(txHash, "confirmed");
  });
  ...
  it("Do Roll Dice!", async () => {
    ...
    try {
      callbackSubId = baseConnection.onLogs(
        programId,
        (info) => {
      ...
      const playerAccount = await baseConnection.getAccountInfo(
        playerPda,
        "confirmed",
      );
      if (!playerAccount) {
      ...
      if (callbackSubId !== undefined) {
        await baseConnection.removeOnLogsListener(callbackSubId);
      }
    }
  });
});
