...
const PUBLIC_ER_ENDPOINT =
  process.env.REACT_APP_EPHEMERAL_PROVIDER_ENDPOINT ||
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      if (!counterProgramClient.current) {
        ...
        const accountInfo =
          await provider.current.connection.getAccountInfo(counterPda);
        if (cancelled) return;
        if (accountInfo) {
          // @ts-ignore
      ...
      const erAccountInfo =
        await ephemeralConnection.current.getAccountInfo(counterPda);
      if (cancelled) return;
      if (erAccountInfo && counterProgramClient.current) {
        const c = counterProgramClient.current.coder.accounts.decode(
          "counter",
          erAccountInfo.data,
        );
  ...
  const submitTransaction = useCallback(
    async (
      ...
      const targetConnection = ephemeral
      ...
      try {
        ...
        if (!ephemeral && !useTempKeypair) {
          signature = await sendTransaction(transaction, targetConnection, {
            minContextSlot,
          });
        } else {
          signature = await targetConnection.sendRawTransaction(
        ...
        if (sigConfirm.value.err) {
          throw new Error(
            `Transaction failed on chain: ${JSON.stringify(sigConfirm.value.err)}`,
          );
        }
        ...
        if (watchAccount && counterProgramClient.current) {
          ...
          try {
            const accountInfo =
              await targetConnection.getAccountInfo(watchAccount);
              ...
              if (ephemeral) setEphemeralCounter(value);
                ...
                setIsDelegated(
                  !accountInfo.owner.equals(
                    counterProgramClient.current.programId,
                  ),
                );
              }
            }
          } catch {
            /* decode may fail mid-transition (e.g., during delegate ownership flip) */
          }
  ...
  const increaseCounterTx = useCallback(async () => {
    ...
    if (!isDelegated) {
      ...
      if (!accountInfo) {
        console.log(
          "Counter not initialized, prepending initialize instruction",
        );
