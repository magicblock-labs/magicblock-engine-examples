...
pub fn process_initialize_counter(program_id: &Address, accounts: &[AccountView]) -> ProgramResult {
    ...
    let permission_program = accounts[3];
    let permission = accounts[4];
    let delegation_buffer = accounts[5];
    ...
    if counter_account.lamports() == 0 {
        ...
        create_account_ix.invoke_signed(&[signer_seeds.clone()])?;
    }
