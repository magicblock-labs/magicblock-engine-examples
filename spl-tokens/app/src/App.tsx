...
const App: React.FC = () => {
    ...
    useEffect(() => {
        ...
        // Default to a specific ER region (devnet-as) — see anchor-counter/app/src/App.tsx
        // about why the router URL (devnet.magicblock.app) breaks WS subscriptions.
    ...
    // confirmTransaction does NOT throw when the tx confirmed-with-error — it resolves with
    // result.value.err set. Wrap it so callers can rely on success === actually succeeded.
    const confirmOrThrow = useCallback(async (conn: Connection, sig: string) => {
    ...
    const ensureAirdropLamports = useCallback(async (conn: Connection, pubkey: PublicKey) => {
        ...
        // Skip airdrop if account already has plenty of SOL — avoids hanging on RPCs without a faucet
        try {
            const balance = await conn.getBalance(pubkey, 'confirmed');
            if (balance >= LAMPORTS_PER_SOL) {
                console.log("[airdrop] skipped", short, "- already has", (balance / LAMPORTS_PER_SOL).toFixed(3), "SOL");
                return;
            }
        } catch (e: any) {
            console.warn("[airdrop] balance check failed for", short, "-", e?.message ?? e);
        }
        // Race the airdrop against a timeout so we don't hang forever on RPCs without a faucet
        const airdropWithTimeout = async () => {
            const signature = await conn.requestAirdrop(pubkey, 2 * LAMPORTS_PER_SOL);
            const { blockhash, lastValidBlockHeight } = await conn.getLatestBlockhash('confirmed');
            await conn.confirmTransaction({ signature, blockhash, lastValidBlockHeight }, 'confirmed');
        };
        const timeout = new Promise<never>((_, reject) =>
            setTimeout(() => reject(new Error("airdrop timed out after 5s — RPC likely has no faucet")), 5000),
        );
        try {
            await Promise.race([airdropWithTimeout(), timeout]);
            console.log("[airdrop] success for", short, "on", conn.rpcEndpoint);
        } catch (e: any) {
    ...
    const refreshBalances = useCallback(async () => {
        ...
        const updated = await Promise.all(accountsRef.current.map(async (acc, idx) => {
            ...
            console.log(
                `[refresh] acct #${idx + 1} (${acc.keypair.publicKey.toBase58().slice(0, 6)}…) ` +
    ...
    const performTransfer = useCallback(async (fromIdx: number, toIdx: number, amountUi: string) => {
        ...
        // "Private" uses the queue/shuttle whenever the path touches base layer.
        // For ER → ER, private collapses to the same direct SPL transfer as public (SDK lines 924/929),
        // so the opts are harmless but unused there.
        const privateInvolvesBase =
            fromBalance === 'base' || toBalance === 'base';
        const usesQueuedPrivateTransfer =
            transferVisibility === 'private' && privateInvolvesBase;
        if (!mint) {
            setTransactionError('Mint not initialized. Run Setup first.');
            return;
        }
        ...
        // Block only when src and dst are the *same pocket* (same account AND same side).
        // Same account / different side is a legitimate deposit-to-self or withdraw-to-self.
        ...
        try {
            ...
            if (usesEphemeralConnection) {
                ...
                await confirmOrThrow(ephemeralConnection.current, sig);
            } else {
    ...
    const setupAll = useCallback(async () => {
        console.log("[setup] clicked. state:", {
            hasEphemeralConnection: !!ephemeralConnection.current,
            ephemeralEndpoint: ephemeralConnection.current?.rpcEndpoint,
            hasValidator: !!validator.current,
            validator: validator.current?.toBase58(),
        ...
        const eConn = ephemeralConnection.current;
        if (!eConn) {
            console.warn("[setup] aborted: ephemeralConnection not yet initialized");
            setTransactionError("Ephemeral connection not initialized — check REACT_APP_EPHEMERAL_PROVIDER_ENDPOINT");
            return;
        }
        setTransactionError(null);
        setTransactionSuccess(null);
        ...
        if (!queueValidator) {
            console.warn(
                "[setup] validator not yet detected. ER endpoint:", eConn.rpcEndpoint,
                "— scheduling retry in 1s. If this loops forever, getIdentity is failing against that endpoint.",
            );
            setTransactionError(`Waiting for validator from ${eConn.rpcEndpoint}... check console for getIdentity errors`);
            if (autoSetupRetryTimeoutRef.current === null) {
                autoSetupRetryTimeoutRef.current = setTimeout(() => {
                    autoSetupRetryTimeoutRef.current = null;
                    setupAll().catch(e => console.error("[setup] retry threw:", e));
                }, 1000);
            }
            return;
        }
        ...
        try {
            ...
            await Promise.all(accounts.map(a => ensureAirdropLamports(connection, a.keypair.publicKey)));

            // Sanity check: payer must have SOL to send mint tx. If RPC has no faucet, user must pre-fund.
            const payerBalance = await connection.getBalance(payer.publicKey, 'confirmed');
            console.log("[setup] payer balance:", (payerBalance / LAMPORTS_PER_SOL).toFixed(4), "SOL on", connection.rpcEndpoint);
            if (payerBalance < LAMPORTS_PER_SOL / 10) {
                const msg = `Payer ${payer.publicKey.toBase58()} has only ${(payerBalance / LAMPORTS_PER_SOL).toFixed(4)} SOL on ${connection.rpcEndpoint}. This RPC has no faucet — send some devnet SOL to that address from your wallet, then retry Setup.`;
            ...
            console.log("[setup] step 2/3: mint generated", mintKp.publicKey.toBase58());
            ...
            const setupOn = async (conn: Connection) => {
                ...
                console.log("[setup]   sending mintTx...");
            ...
            console.error("[setup] failed during execution:", e);
            if (e?.logs) console.error("[setup] program logs:", e.logs);
            if (e?.getLogs) {
                try {
                    console.error("[setup] e.getLogs():", await e.getLogs(connection));
                } catch (logErr) {
