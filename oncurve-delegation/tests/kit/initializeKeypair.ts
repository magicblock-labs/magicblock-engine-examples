...
> {
  ...
  if (!process.env.PRIVATE_KEY) {
    ...
    keypair = await createKeyPairFromBytes(new Uint8Array(secret));
  }
  return keypair;
}

// Initialize Keypair for fee payer and signer
export async function initializeFeePayer(
  connection: Connection,
  userKeypair: CryptoKeyPair,
): Promise<ReturnType<typeof createKeyPairFromPrivateKeyBytes>> {
  ...
  if (!process.env.FEE_PAYER_PRIVATE_KEY) {
    ...
    feePayerKeypair = await createKeyPairFromBytes(new Uint8Array(secret));
  }

  // Transfer 0.1 SOL to fee payer if needed
  const userAddress = await getAddressFromPublicKey(userKeypair.publicKey);
  const feePayerAddress = await getAddressFromPublicKey(
  ...
  if (
    feePayerBalance < TRANSFER_AMOUNT &&
    userBalance >= 2n * TRANSFER_AMOUNT
  ) {
...
export async function cryptoKeyPairToTransactionSigner(
