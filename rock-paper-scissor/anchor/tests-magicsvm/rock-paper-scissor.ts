...
function fmtResult(result: any, p1: PublicKey, p2: PublicKey): string {
  if (!result) return "—";
  if ("tie" in result) return "🤝 Tie";
  if ("none" in result) return "⏳ Not yet revealed";
  if ("winner" in result) {
    const winner: PublicKey = result.winner["0"];
...
function printGameResult(game: any) {
  const p1 = game.player1 as PublicKey;
  const p2 = game.player2 as PublicKey;
  console.log("┌─────────────────────────────────────────────");
