...
describe("magic-router-and-multiple-atomic-ixs", () => {
  ...
  const connection = new ConnectionMagicRouter(
    process.env.PROVIDER_ENDPOINT || "https://devnet-router.magicblock.app",
    {
      wsEndpoint:
        process.env.WS_ENDPOINT || "wss://devnet-router.magicblock.app",
    },
  );
  ...
  it("Increase delegated counter and commit through CPI", async function () {
    const start = Date.now();
